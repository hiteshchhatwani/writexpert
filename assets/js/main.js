...
(function () {
  ...
  (function () {
    ...
    function genRays() {
      rays = [];
      var N = 150, startA = Math.PI * 200 / 180, endA = Math.PI * 340 / 180;
      for (var i = 0; i < N; i++) {
        var t = i / (N - 1);
        rays.push({ ang: startA + t * (endA - startA), len: 0.5 + Math.random() * 0.5, dots: 1 + Math.floor(Math.random() * 3), jitter: Math.random() });
      }
    }
    function draw() {
      var th = themes[current], W = canvas.width, H = canvas.height;
      ctx.clearRect(0, 0, W, H);
      var ox = W * 0.5, oy = H * 0.99, maxLen = H * 0.95;
      for (var i = 0; i < rays.length; i++) {
        var ray = rays[i], col = th.colors[i % th.colors.length];
        var ex = ox + Math.cos(ray.ang) * maxLen * ray.len, ey = oy + Math.sin(ray.ang) * maxLen * ray.len;
        ...
        ctx.globalAlpha = 0.6 + ray.jitter * 0.4; ctx.fillStyle = th.dot;
        ctx.beginPath(); ctx.arc(ex, ey, dpr * 1.6, 0, Math.PI * 2); ctx.fill();
        for (var d = 0; d < ray.dots; d++) {
          var f = (d + 1) / (ray.dots + 1), mx = ox + (ex - ox) * f, my = oy + (ey - oy) * f;
          ctx.globalAlpha = 0.35; ctx.beginPath(); ctx.arc(mx, my, dpr * 1.0, 0, Math.PI * 2); ctx.fill();
        }
      }
      ctx.globalAlpha = 1;
    }
    function resize() { var r = canvas.getBoundingClientRect(); canvas.width = r.width * dpr; canvas.height = r.height * dpr; draw(); }
    function setMode(m) {
  ...
  (function () {
    ...
    function open() { panel.classList.add('open'); if (!greeted) { addMsg("Hi! I'm writeXpert's assistant. Ask me about what we build, our process, or how to start a project.", 'bot'); greeted = true; } input.focus(); }
